...
def redefine_et_ed_labels(seg_file, out_file, label="et", ratio=0):
...
def remove_disconnected(seg_file, out_file, t_ncr=50, t_ed=50, t_et=50):
    seg_obj = nib.load(seg_file)
...
def remove_disconnected_from_dir(input_dir, output_dir, t_ncr=50, t_ed=50, t_et=50):
    ...
    for seg1 in input_dir.iterdir():
        if seg1.name.endswith('.nii.gz'):
            ...
            ncr, n_ncr, ed, n_ed, et, n_et = remove_disconnected(
