...
#     return mask_path, f"Segmentation done! Total tumor volume segmented {mydict.get('vol_lbl3', 0) + mydict.get('vol_lbl2', 0) + mydict.get('vol_lbl1', 0):.3f} ml; EDEMA {mydict.get('vol_lbl2', 0):.3f} ml; ENHANCING TUMOR {mydict.get('vol_lbl3', 0):.3f} ml; NECROSIS {mydict.get('vol_lbl1', 0):.3f} ml"
...
#         annotations = [
#             (slice_mask == 1, f"necrosis: {mydict.get('vol_lbl1', 0):.3f} ml"),
#             (slice_mask == 2, f"edema: {mydict.get('vol_lbl2', 0):.3f} ml"),
#             (slice_mask == 3, f"enhancing tumor: {mydict.get('vol_lbl3', 0):.3f} ml")
#         ]
...
with gr.Blocks() as demo:
    ...
    gr.HTML(value=f"<justify><font size='5'> Thank you for your interest in the pediatric brain tumor segmentation tool. The current model is undergoing a major update. It is expected to be online by second week of September 2024.</font></justify>")
    gr.HTML(value=f"<justify><font size='4'> To stay updated with the different model updates sign up <a href='https://forms.gle/e634eJzoimhHnJ7W9'>here</a>.</font></justify>")
